...
class LatestAds extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final adsController = Provider.of<LatestAdsController>(context);
    final size = MediaQuery.of(context).size;
    final orientation = MediaQuery.of(context).orientation;

    // Adjust dimensions based on orientation
    final containerHeight = orientation == Orientation.portrait
        ? size.height * 0.30 // Portrait height
        : size.height * 0.7; // Landscape height

    final cardWidth = orientation == Orientation.portrait
        ? size.width * 0.7 // Portrait card width
        : size.width * 0.4; // Landscape card width

    final imageHeight = orientation == Orientation.portrait
        ? containerHeight * 0.5 // Portrait image height
        : containerHeight * 0.6; // Landscape image height

    return LayoutBuilder(
      builder: (context, constraints) {
        return StreamBuilder<LatestAdsModel?>(
          stream: adsController.dataStream,
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return _buildShimmerLoader(
                ...
                !snapshot.hasData ||
                snapshot.data!.ads.isEmpty) {
              return const NoAdsModernWidget();
            }
